...
def test(args, io):   #测试训练的结果
    ...
    #将总的测试数据分为每份batch_size,例如本次就以16为test_batch_size,将2468份样本分为155组，前154每份16份样本，最后一组4份样本
    for data, label in test_loader:
        ...
        preds = logits.max(dim=1)[1]  #preds 的形式为tensor([17, 12, 12, 22, 27, 17,  1, 37, 26,  8, 17, 33,  7, 17, 22, 22]，device='cuda:0') batch_size大小的数组，里面是预测分类值  取最大的值对应的类别索引作为预测的分类
